...
class Plugin {
  ...
  constructor(node) {
    ...
    this.ethereum = new Ethereum({
      ...
      customEnsAddress: config.str('handover-ens-address'),
    ...
    this.ns.middle = async (tld, req) => {
      ...
      for (const rr of res.authority) {
        ...
        if(rr.data.ns.endsWith('._eth_aliased.')) {
          hasEnsReferral = true;
          ...
          this.logger.debug(
            'Intercepted referral to ._eth_aliased: %s %s -> %s NS: %s',
            name,
            wire.typesByVal[type],
            rr.name,
            rr.data.ns
          );
          const alias = rr.data.ns.split('.')[0];
          const node = name.replace(new RegExp(`${tld}$`), `${alias}.`);
          data = await this.ethereum.resolveDnsFromAliasedEns(
            name,
            type,
            rr.data.ns,
            node
      ...
      try {
        ...
        this.logger.warning(`unable to sign RR set of type ${type}: ${JSON.stringify(replacementRes.answer)}`)
      }
