...
class Ethereum {
  ...
  constructor(options) {
    ...
    switch (options.provider) {
      case 'infura':
        provider = new ethers.providers.InfuraProvider(options.network || 'homestead', options);
        ensAddress = options.customEnsAddress || MAINNET_ENS_ADDRESS;
        break;
      case 'jsonrpc':
  ...
  async resolveDnsFromAliasedEns(name, type, ns, node) {
    if (!node)
      node = name;
    ...
    const [alias, addr, suffix] = labels;

    if (suffix !== '_eth_aliased')
      return null;
